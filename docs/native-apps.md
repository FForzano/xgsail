...
- **watchOS target sources**: `frontend/ios/XGSailWatch/` (SwiftUI recorder +
  WatchConnectivity client). It is a hand-added watchOS App target on
...
- **Phone plugin**: `frontend/ios/App/App/WatchBridgePlugin.swift` (+ `.m`) is
  ...
  ios`; no extra phone entitlement is needed.
...
- **Testing**: on-device only (a paired watch + paid account). Claim the watch,
  record a short session, stop, and confirm the files transfer to the phone and
  the session appears with a GPS track and a `heart_rate` stream tied to the
  user.
