...
track from the phone, including with the screen locked, sampling at most once
a second (`frontend/src/services/nativeRecording.ts`). This needs
...
3. Sampling cadence is enforced in JS, not by the plugin: the watcher
   ...
   `SAMPLE_INTERVAL_MS` (1s) has elapsed since the last saved one, since
   the plugin itself has no fixed-cadence mode. The same throttle feeds the
   live navigation display, so the track and the display see identical points.
4. Recordings are stored locally first (a newline-delimited point log under
   ...
   `solo`) sessions only.
5. A recording's *running* state (the `active` handle + the plugin watcher)
   lives only in memory; the on-disk index is the durable part. So an entry
   read back as `recording`/`paused`/`uploading` at startup can only be the
   leftover of a process that died — the OS or an OEM battery manager killing
   the app mid-track, or a crash mid-upload. `reconcileOrphans()` settles
   those on the first index load: the raw log is finalized into a `.gpx`,
   `endedAt` is set to the **last recorded fix** (not "now", or the row would
   report the age of the orphan as its duration), and the status becomes
   `interrupted` — a terminal state offering upload (when at least one point
   was captured) and delete, never auto-uploaded. `stop()` settles the same
   way when a recording ends with zero fixes. Without this, such an entry was
   unreachable: `stop()` no-ops without an in-memory `active`, and the upload
   retry only picks up `stopped`/`failed`.

## Apple Watch companion (watchOS)

A native watchOS app records a sailing session's GPS + HealthKit signals and
relays them through the paired iPhone, which uploads them via the device
protocol (`docs/device-protocol.md` §9). It is **iOS-only**, native Swift (not
Capacitor/web), **not** OTA-shippable, and needs a **paid Apple Developer
account** to build to a device — the same gate as the Share Extension above.

- **watchOS target sources**: `frontend/ios/App/XGSailWatch Watch App/`
  (SwiftUI recorder + WatchConnectivity client) — on Xcode 16+ a new watchOS
  target is backed by a filesystem-synchronized folder next to
  `App.xcodeproj`, so the real files live there rather than a standalone
  `ios/XGSailWatch/` directory. It is a hand-added watchOS App target on
  `App.xcodeproj`, not generated by `cap sync` — see that folder's
  `README.md` for the one-time Xcode setup (target creation, HealthKit +
  Workout-processing background mode, watch `Info.plist` health/location usage
  strings).
- **Phone plugin**: `frontend/ios/App/App/WatchBridgePlugin.swift` (+ `.m`) is
  a local Capacitor plugin (`WatchBridge`) that owns the phone's `WCSession`:
  it stores the files the watch transfers under `Directory.Data` and emits
  `watchSessionReceived`. Add both files to the `App` target after `cap add
  ios`; no extra phone entitlement is needed. This Capacitor version doesn't
  auto-discover local (non-npm) plugins via Objective-C runtime scanning, so
  it also needs explicit registration — `frontend/ios/App/App/BridgeViewController.swift`
  (a `CAPBridgeViewController` subclass registering it in `capacitorDidLoad()`)
  wired up as `Main.storyboard`'s view controller class. Skipping this step
  makes every `WatchBridge.*` call from JS fail with `"WatchBridge" plugin is
  not implemented on ios`.
- **Web/TS glue** (all native-only, no-op on web): `services/nativeWatch.ts`
  (claim the watch as a `wearable` device, relay received bundles),
  `services/deviceUpload.ts` (the shared device-protocol open→PUT→PATCH used
  by the relay), `plugins/watchBridge.ts` (the plugin's TS surface), and the
  `useWatchRelay` hook mounted in `AppShell` (the event-driven counterpart to
  `useE1AutoSync` — the watch pushes finished sessions rather than being
  polled). Pairing is the "Apple Watch" card in `AddDeviceDialog` →
  `WatchClaimDialog`, shown only on iOS.
- **Attribution**: the watch's GPS uploads as the boat track
  (`subject_type=boat`) so a single-hander with no onboard tracker still gets
  a track; heart rate / energy / HRV / respiration upload as the wearer's own
  (`subject_type=crew_member`), merged into the *same* boat session by boat +
...
- **Testing**: the Simulator can validate build/UI/capabilities, plugin
  ...
  iPhone + Watch simulator pair with `xcrun simctl pair <watch-udid>
  <phone-udid>` first, since two independently-launched simulators aren't
  paired by default. `WatchBridgePlugin.swift`'s `isSupported`/`isPaired`
  bypass real `WCSession.isPaired`/`isWatchAppInstalled` checks under `#if
  targetEnvironment(simulator) && DEBUG` — those two flags stay false in
  Simulator even with `simctl`-paired devices, so without the bypass the
  "Apple Watch" card in `AddDeviceDialog` never leaves "Not paired", compiled
  out of every device/Release build. What the Simulator can't validate:
  `WCSession.transferFile` (the actual recorded CSV bundle watch→phone) is
  unreliable there regardless of pairing, and there's no real HealthKit sensor
  data. Full verification — a real file transfer landing, a GPS track +
  `heart_rate` stream tied to the user in a session — needs a paired watch on
  a paid account. Claim the watch, record a short session, stop, and confirm
  the files transfer to the phone and the session appears with both streams.
