 ...
 * and @[label](user|club|group:id) mentions. Deliberately not full Markdown —
 * just enough to cover the formatting the composer toolbar can produce, kept
 * as plain text end to end (no HTML/sanitizer needed) so `body` stays a
 * simple string both in the DB and over the wire. Newline preservation is
 * handled by the `white-space: pre-wrap` on `.sf-feed__post-body`, not here. */
const INLINE_RE =
...
export function renderPostBody(body: string): ReactNode {
  ...
  while ((m = INLINE_RE.exec(body))) {
    ...
    const [, mentionLabel, mentionType, mentionId, linkLabel, linkUrl, bold, underline, italic] = m;
