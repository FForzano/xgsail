 ...
 * event; this is the one block that is about the person reading it — hence the
 * single highlighted strip row inside an otherwise chrome-less section. */
export function MyRegattaCard({
  ...
  if (!boatId) {
    return (
      <Section title={t("regate.yourRegatta")}>
  ...
  return (
    <Section title={t("regate.yourRegatta")}>
      {sailed === 0 || !row ? (
        ...
        <div className="sf-strip__item sf-strip__item--active">
